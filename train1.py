...
def main(args):
    ...
    elif args.tokenizeModel == "bert-base-chkill inese":
        tokenizer = BertTokenizer.from_pretrained(args.tokenizeModel)
    ...
    # set the parameter
    pre_weight_p, pre_bias_p = [], []
    for name, p in preModel.named_parameters():
        if 'bias' in name:
            pre_bias_p += [p]
        else:
            pre_weight_p += [p]
    class_weight_p, class_bias_p = [], []
    for name, p in classifier.named_parameters():
        if 'bias' in name:
            class_bias_p += [p]
        else:
            class_weight_p += [p]
    myParameter = [{"params": pre_weight_p, 'weight_decay': 1e-5}, {"params": pre_bias_p, 'weight_decay': 0},
                   {"params": class_weight_p, 'weight_decay': 1e-5}, {"params": class_bias_p, 'weight_decay': 0}]
