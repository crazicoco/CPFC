...
os.environ['CUDA_VISIBLE_DEVICES'] = '5,0,3'
...
def adjust_learning_rate(base_lr, optimizer, epoch):
    ...
    for param_group in optimizer.param_groups:
        param_group['lr'] = base_lr * (0.5 ** (epoch))
...
def main(args):
    ...
    for name, p in classifier.named_parameters():
        if 'bias' in name:
            ...
            class_weight_p += [p]
    myParameter = [{"params": pre_weight_p, 'weight_decay':1e-4}, {"params": pre_bias_p, 'weight_decay':0},
                   {"params": class_weight_p, 'weight_decay':1e-4}, {"params": class_bias_p, 'weight_decay':0}]
    ...
    lossProcess = LossObeject(lossCalculateWay=args.lossCalculateWay)
    best_dev_f1 = 0.0
    best_epoch = -1
