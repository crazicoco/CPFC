...
# @Author        : crazicoco
# @Time          : 2021/1/4 13:21
# @Function      : use the 2.0version 权重更新
import os
...
def main(args):
    ...
    labelWeight = [[8.51, 7.14, 6.25, 5.80, 7.02, 6.35, 9.10], [3.01,3.01,2.999], [20.98, 17.98, 15.73, 11.59, 19.15, 13.77, 12.24, 14.44, 15.46, 16.31, 16.94, 16.94, 10.74, 16.62, 13.15]]
    labelWeight_sum = [sum(item) for item in labelWeight]
    labelWeight_min = [min(item) for item in labelWeight]
    labelWeight_sum = [labelWeight_sum[idx] - labelWeight_min[idx] * labelNums[idx] for idx in range(len(labelNums))]
    min_idx = [idx for j in range(len(labelNums)) for idx in range(len(labelWeight[j])) if labelWeight_min[j] == labelWeight[j][idx] ]
    for j in range(len(labelNums)):
        labelWeight[j] = [labelWeight_sum[j] / (labelWeight[j][idx] - labelWeight[j][min_idx[j]]) if idx != min_idx[j] else 50000 for idx in range(labelNums[j])]
    temp_min = [min(item) for item in labelWeight]
    for j in range(len(labelNums)):
        labelWeight[j][min_idx[j]] = temp_min[j]
        labelWeight[j] = torch.tensor(labelWeight[j])
    if args.device != '-1':
        for j in range(len(labelNums)):
            labelWeight[j] = labelWeight[j].cuda()
    ...
    for epoch in range(args.epoch_size):
        ...
        while(True):
            ...
            if accumulate and (cntTrain + 1) % a_step == 0:
                ...
                # # labelWei_oce = get_new_weight(classification_report(y_true=labelOcemotionList, y_pred=preOcemotionList, labels=list(range(labelNums[0]))), labelNums[0], args.device, args.a_step, labelWeight[0])
                # # labelWei_ocn = get_new_weight(classification_report(y_true=labelOcnliList, y_pred=preOcnliList, labels=list(range(labelNums[1]))), labelNums[1],args.device, args.a_step, labelWeight[1])
                # # labelWei_tne = get_new_weight(classification_report(y_true=labelTnewsList, y_pred=preTnewsList, labels=list(range(labelNums[2]))), labelNums[2], args.device, args.a_step, labelWeight[2])
                # # temp = (labelWei_oce, labelWei_ocn, labelWei_tne)
...
def get_new_weight(report_task, labelNum, device, a_step, baseline_wei=None):
    """
    1.0版本所有train和valid使用同一个方式进行优化，math.log，防止前后差值过大，导致无法收敛
    1.1版本发现加入valid后反而使得优化效果不好，发现可能是math.log弱化了当前的结果
    1.2版本 发现需要增强权值的差距，有效果
    2.0版本提供权重更新方式：
        每次减去最小值，然后计算当前的权值，其中最小值由倒数第二的权值代替。
        理论：loss每次优化过程中权值不参与更新，所以可以收敛
        每次train不再随机更变权值，而是由验证集的结果去更新loss优化的结果，其实每次train也只是不断去拟合结果，反而有可能无法达到最优结果。
    3.0
    :param report_task:
    :param labelNum:
    :param device:
    ...
    temp_min = min(temp_precise)
    # use math.log
    # labelWei = [math.log(temp_sum / temp_precise[i]) for i in range(labelNum)]
    # not use math.log
    # labelWei = [temp_sum / temp_precise[i] for i in range(labelNum)]
    # use the 2.0 function
    temp_sum = temp_sum - temp_min * labelNum
    idx = [idx for idx in range(labelNum) if temp_precise[idx] == temp_min]
    labelWei = [temp_sum / (temp_precise[i]-temp_min) if i not in idx else 500000 for i in range(labelNum)]
    wei_min = min(labelWei)
    labelWei = [wei_min if i in idx else labelWei[i] for i in range(labelNum)]
