...
def main(args):
    ...
    # set the parameter
    # TODO 固定embedding层不进行更新
    # for name, p in preModel.named_parameters():
    #     if name.find("word_embeddings") != -1:
    #         p.requires_grad = False
    # myParameter = [{"params": filter(lambda x: x.requires_grad is not False, preModel.parameters()), }, {"params": classifier.parameters()}]
    ...
    lossProcess = LossObeject(args.lossCalculateWay)
    best_dev_f1 = 0.0
    best_epoch = -1
