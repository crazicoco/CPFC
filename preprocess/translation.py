...
def readAndWrite2(index1, fileReadPath, fileWritePath, srcLang, destLang):
    with open(fileReadPath, 'r', encoding='utf8') as fileread:
        with open(fileWritePath, 'a+', encoding='utf8') as filewrite:
            sum = 3000
            for idx,line in enumerate(fileread.readlines()):
...
def main():
    ...
    # TODO test chinese to english
    readAndWrite2(2, os.path.join(root, filePathEnglish[2]), os.path.join(root, filePathChinese[2]), english, baidu_chinese)
    # for i in range(0,fileNum):
    #     readAndWrite2(i, os.path.join(root, filePathEnglish[i]), os.path.join(root, filePathChinese[i]), english, baidu_chinese)
